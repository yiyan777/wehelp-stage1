from fastapi import FastAPI, Form, HTTPException, Request, Path, Query, Body
...
@app.get("/api/member")
async def get_member(username: str = Query(None, description="會員帳號")):
    con = mysql.connector.connect(
        user="root",
        password="123456",
        host="localhost",
...
async def nameUpdate(request: Request, data: dict = Body(...)):
    member_id = request.session.get("member_id") #確保會員登入
    if not member_id:
        return JSONResponse(content={"error": True}, status_code=401)
    
    newName = data.get("name") #從JSON解析新名字
    if not newName:
    ...
    request.session["name"] = newName #更新session
    return {"ok": True}